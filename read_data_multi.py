...
def get_padded_seq_lens(padded_enc_w2v, padded_target):
    ...
    target_lens = np.asarray([(target_i != kb_pad_idx).sum(axis=0) for target_i in padded_target])
    
    return enc_w2v_lens, target_lens
...
def get_batch_data(data_dict, max_len=20, max_utter=1, max_target_size=10, batch_size=64, max_mem_size=2000, overriding_memory=None, is_test=False):
    ...
    max_mem_size = max_mem_size + 1 # 1 for oov ent.
    ...
    enc_w2v_lens, target_lens = get_padded_seq_lens(padded_enc_w2v, padded_target)
    #enc_w2v_lens = get_padded_seq_lens(padded_enc_w2v)
    
    #padded_response_mask = get_mask(padded_response)
    
    return padded_enc_w2v, enc_w2v_lens, padded_enc_kb, padded_target, target_lens, padded_orig_target, batch_orig_response, padded_memory_weights, padded_batch_sources, padded_batch_rel, padded_batch_key_target
        ...
        for i in range(padded_target.shape[0]):
            for j in range(padded_target.shape[1]):
                ...
                if padded_target[i,j] in batch_key_target[i,:] and padded_target[i,j] != kb_pad_idx:
                    ...
                    key_target_indexes = np.nonzero(batch_key_target[i,:] == padded_target[i,j])[0]
                    for key_target_i in key_target_indexes:
                        mapped_padded_target[i,key_target_i] = 1 
